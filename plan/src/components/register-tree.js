...
export default function (cytoscape) {
    ...
    cytoscape('collection', 'isRoot', function () {
        return this.incomers().length == 0
    })
    cytoscape('collection', 'isLeftChild', function () {
        return this.data('treepos') == 'left'
    })
    cytoscape('collection', 'isRightChild', function () {
        return this.data('treepos') == 'right'
    })
    cytoscape('collection', 'parent', function () {
        return this.incomers('node')
    })
    cytoscape('collection', 'leftChild', function () {
        return this.outgoers('node[treepos="left"]')
    })
    cytoscape('collection', 'rightChild', function () {
        return this.outgoers('node[treepos="right"]')
    })
    cytoscape('collection', 'parentEdge', function () {
        return this.incomers('edge')
    })
    ...
    TreeLayout.prototype.compute = function () {
        ...
        // Determine root node
        console.log(options)
        var root = options.cy.nodes().filter(node => node.indegree() == 0)
        if (root.length !== 1)
    ...
    TreeLayout.prototype.run = function () {
        ...
        var options = this.options
        options.cy.nodes().layoutPositions(this, options, function (node) {
            return positions[node.id()]
        })
